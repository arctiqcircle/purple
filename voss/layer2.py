...
from base.network_objects import Port, MacAddresses
from voss import TechFile


@TechFile.parser("show lldp neighbor")
def get_lldp_neighbors(text_lines: list[str]) -> dict[Port, dict[str, Any]]:
    """
    ...
    for line in text_lines:
        ...
        if re.search(r'Port: *\d+/\d+', line):
            ...
            data[port]["LLDP Remote Port"] = Port(line.split()[-1].strip())
            pass
    return data


@TechFile.parser("show isis adjacencies")
def get_isis_adjacencies(text_lines: list[str]) -> dict[Port, dict[str, Any]]:
    """
    ...
    for line in text_lines:
        ...
        if port_name:
            ...
            data.update({port: {"ISIS Adjacency": adj, "ISIS Status": status == "ACTIVE"}})
    return data
